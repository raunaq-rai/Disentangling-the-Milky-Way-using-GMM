...
\noindent\textbf{XP $\alpha$–abundances and cross-match.}  
We obtain $[\alpha/\mathrm{M}]$ from the neural-network catalogue of
\citet{Li2024}, which is trained on \textsc{APOGEE}~DR17 and predicts XP
spectra simultaneously, achieving a typical precision of
$\sim$0.05 dex for $G<16$.  Cross-matching this list with the
metallicity-selected red-giant sample of
Section~\ref{subsec:data_sample}—and applying the same parallax,
reddening, latitude and sub-structure masks—returns
$9.6\times10^{6}$ giants that now carry homogeneous values of
$[\mathrm{M/H}]$, $[\alpha/\mathrm{M}]$ and full 6-D phase-space
information.

\vspace{0.5\baselineskip}
\noindent\textbf{Separating high- and low-$\alpha$ discs.}  
Figure~\ref{fig:alphametal} shows the density of stars in the
$[\alpha/\mathrm{M}]$–$[\mathrm{M/H}]$ plane.  In a similar fashion to 
\citet{Chandra_2024} we isolate the two sequences with the piece-wise 
linear boundary
...
\begin{aligned}
  \text{high-}\alpha:\;&
   \begin{cases}
     [\mathrm{M/H}]<-0.60 & [\alpha/\mathrm{M}]>0.28\\
     -0.60\le[\mathrm{M/H}]\le0.125
       & [\alpha/\mathrm{M}]>-0.25[\mathrm{M/H}]+0.13\\
     [\mathrm{M/H}]>0.125 & [\alpha/\mathrm{M}]>0.10
   \end{cases}\\[3pt]
  \text{low-}\alpha:\;&
   \begin{cases}
     [\mathrm{M/H}]<-0.80 & [\alpha/\mathrm{M}]<0.21\\
...
with the transition zone between the cuts (shaded purple in
Fig.~\ref{fig:alphametal}) discarded.  The resulting
high-$\alpha$ and low-$\alpha$ subsamples contain 2.2 million and
7.4 million stars, respectively.  Although the high-$\alpha$ cohort is
often equated with \emph{in-situ} formation, we emphasise that chemical
tags alone do not guarantee such a distinction; a detailed discussion
appears in Section~\ref{sec:discussion}.

\vspace{0.5\baselineskip}
\noindent\textbf{Context.}  
The chemical bifurcation dovetails with the “three-phase’’ picture of
Milky-Way assembly proposed by \citet{Chandra_2024}: an early chaotic,
high-$\alpha$ phase; a rapid spin-up between
$-1.3\lesssim[\mathrm{M/H}]\lesssim-0.9$; and subsequent thin-disc
growth.  By combining $[\alpha/\mathrm{M}]$ with metallicity we can now
track the emergence of ordered rotation across \emph{both} chemical
dimensions in our mixture-model analysis (Section~\ref{sec:methods}).
