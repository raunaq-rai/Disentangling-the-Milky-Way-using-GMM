...
We revisit the suggestion that the Milky Way contains a rotation–supported disc at very 
low metallicity by reproducing and extending the extreme–deconvolution Gaussian-mixture 
analysis of \citet{zhang2024existencemetalpoordiscmilky}.  Our starting point is the bright 
red-giant catalogue from \textit{Gaia}~DR3, supplemented with homogeneous metallicities 
from \citet{Andrae2023} and alpha-element abundances from the XP survey of \citet{Li2024}. 
In each metallicity bin across \(-3.0<\mathrm{[M/H]}<-1.0\) we fit the full three-dimensional
velocity distribution with a Gaussian mixture, letting the Bayesian information criterion 
select the optimum number of components.

With the full sample we confirm the original result: below \(\mathrm{[M/H]}\simeq-1.6\) 
the data require only two halo Gaussians, one stationary and one mildly prograde 
\((v_{\phi}\simeq68\,\mathrm{km\,s^{-1}})\).  No component satisfies 
\(v_{\phi}/\sigma_{\phi}>1\); any thick-disc contribution in the very-metal-poor interval 
\(-3.0<\mathrm{[M/H]}<-1.6\) must therefore be negligible.

Between \(-2.0<\mathrm{[M/H]}<-1.6\) the mixture broadens to four Gaussians: the stationary 
halo, the prograde halo and a symmetric pair of highly radial Gaia–Sausage/Enceladus (GS/E) 
components.  These GS/E fragments together account for about fifteen per-cent of the stars 
yet still show no evidence for cold rotation.

Splitting the sample by alpha abundance changes the picture.  In the high-\(\alpha\) branch 
a thick-disc Gaussian first appears at \(\mathrm{[M/H]}\simeq-1.5\) and exceeds fifty 
per-cent weight by \(\mathrm{[M/H]}\simeq-1.1\).  Low-\(\alpha\) stars spin up later: a 
warm disc component of roughly twenty-five per-cent is detected only in the bin 
\(-1.3<\mathrm{[M/H]}<-1.0\), while lower-metallicity bins remain halo-dominated.  
Surprisingly, a GS/E-like component is recovered inside the high-\(\alpha\) subset; 
its presence is almost certainly due to mis-classified, low-signal-to-noise abundances 
rather than a genuinely alpha-rich progenitor.

Taken together, these results indicate that ordered rotation became significant once the 
interstellar medium reached about one-tenth of solar metallicity, and that the alpha-rich 
population settled into a thick disc earlier than the alpha-poor one.
