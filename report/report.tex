...
We analyse six dimensional phase space data for about three point four million bright 
red giant stars from \textit{Gaia} DR3.  Metallicities come from the machine learning 
catalogue of \citet{Andrae2023}, while alpha to metal ratios are taken from the XP 
based study of \citet{Li2024}.  First, we reproduce the extreme deconvolution Gaussian
 mixture modelling of \citet{zhang2024existencemetalpoordiscmilky}, recovering the 
 same stationary and prograde halo components and confirming that no strongly 
 rotating population is required below $\mathrm{[M/H]}\simeq-1.6$.  We then extend 
 the method to the chemically defined high alpha and low alpha sequences introduced 
 by \citet{Vis2024}.  The optimal number of Gaussian components in each metallicity 
 bin is selected with the Bayesian information criterion, allowing a data driven 
 decomposition of the velocity distribution.  

For the high alpha stars a rotating thick disc Gaussian appears at 
$\mathrm{[M/H]}\simeq-1.5$ and gains more than fifty per cent weight by 
$\mathrm{[M/H]}\simeq-1.1$.  The low alpha stars start to spin up at similar 
metallicity but remain dominated by halo and Gaia Sausage Enceladus debris until 
$\mathrm{[M/H]}\gtrsim-1.2$, after which a colder disc like component emerges.  
An unexpected Gaia Sausage Enceladus signature is also detected inside the high 
alpha sample, probably reflecting mis classified low signal to noise abundances 
rather than a genuinely alpha rich progenitor.  

Our results confirm that ordered rotation in the Milky Way began once the interstellar 
medium reached about one tenth solar metallicity and show that the alpha rich 
population settled into a thick disc earlier than the alpha poor one.  Selection 
effects that exclude the low latitude thin disc and the simplifying Gaussian 
assumption are discussed as the main limitations of the present work.
