...
To sharpen the physical picture that the present XD–GMM work has revealed, the next priority is to
replace descriptive Gaussians with fully dynamical models.  Action–based or distribution-function
fitting—implemented with \textsc{RoadMapping} or the \textsc{Agama} library—will capture asymmetric
wings and resonant features, translating the mixture components into the Galaxy’s underlying
potential.  At the same time, a cross-match with high-resolution spectra from APOGEE, GALAH and the
forthcoming WEAVE/4MOST surveys should push the $\alpha$-abundance precision below
$0.03\,\mathrm{dex}$, erasing the present high/low-$\alpha$ overlap and locking the spin-up
thresholds to within $\pm0.1$\,dex.  Forward-modelling Gaia’s magnitude, colour and completeness
cuts will then convert mixture weights into unbiased stellar-mass fractions, yielding the first
robust star-formation-efficiency curve for the thick and thin discs.  Finally, feeding our
metallicity–$\alpha$ thresholds into Milky-Way zoom simulations such as \textsc{Auriga} (MHD) and
\textsc{FIRE-2} (hydrodynamic) will allow us to track thick-/thin-disc analogues in a cosmological
context and to generate mock JWST and Rubin LSST observations, providing testable predictions for
external spiral galaxies.

\section{Conclusion}

We have reproduced the extreme-deconvolution Gaussian-mixture analysis of
\citet{zhang2024existencemetalpoordiscmilky} with Gaia DR3 red giants, confirming that the Milky Way
contains no rotation-supported disc below ${\rm [M/H]}\!\simeq\!-2$ and that disc-like kinematics
first emerge at ${\rm [M/H]}\!\simeq\!-1.6$.  By introducing XP-based $\alpha$ abundances
\citep{Li2024} and adopting the high-/low-$\alpha$ split of \citet{Vis2024}, we have shown that the
high-$\alpha$ sequence spins up at ${\rm [M/H]}\!\approx\!-1.6$—marking the thick-disc onset—while
the low-$\alpha$ sequence remains halo-like until ${\rm [M/H]}\!\approx\!-1.3$, signalling the birth
of the thin disc.  A tentative GS/E-like signal inside the high-$\alpha$ branch is probably a
by-product of low-S/N abundance contamination rather than a genuine $\alpha$-rich remnant.  Together
these results map a clear, two-phase timeline for disc assembly and set precise chemical–kinematic
benchmarks for future dynamical modelling and survey targeting. 
