...
Both samples are modelled by a \emph{single}, non–rotating halo Gaussian
...
\paragraph{Intermediate–metallicity regime (\textbf{IMP}\,: $-2.0<[\mathrm{M/H}]<-1.6$).}
Here the low-$\alpha$ population already splits into
\emph{stationary} and \emph{prograde} halo Gaussians of comparable
...
\emph{earlier} for $\alpha$–poor stars, but remains dynamically hot.

\paragraph{Disc onset (\textbf{MP1}\,: $-1.6<[\mathrm{M/H}]<-1.3$).}
...
\paragraph{Transition to disc dominance (\textbf{MP2}\,: $-1.3<[\mathrm{M/H}]<-1.0$).}
...
From this analysis we infer that the \emph{high}-$\alpha$ sequence evolves from a dispersion–supported 
halo to a kinematically warm \emph{thick disc} between $[\mathrm{M/H}]\simeq-1.6$ and $-1.3$, whereas 
the \emph{low}-$\alpha$ sequence starts to gain rotation at slightly lower metallicity yet remains 
